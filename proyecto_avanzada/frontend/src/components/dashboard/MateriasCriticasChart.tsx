...
export function MateriasCriticasChart({ data }: { data: MateriaCritica[] | undefined }) {
  if (!data?.length) return (
    <Card className="border-none shadow-md">
      <CardHeader><CardTitle className="text-base font-semibold">Materias Críticas</CardTitle></CardHeader>
      <CardContent><p className="text-muted-foreground text-sm">No hay materias con promedio menor a 70.</p></CardContent>
    </Card>
  );
  return (
    <Card className="border-none shadow-md">
      ...
      <CardContent>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data} layout="vertical" margin={{ top: 5, right: 20, bottom: 5, left: 80 }}>
            ...
            <ReferenceLine x={70} stroke="hsl(0,72%,51%)" strokeDasharray="4 4" label={{ value: "70", fill: "hsl(0,72%,51%)", fontSize: 11 }} />
